...
fish|>
  map_int(.cols=everything(), .f = ~sum(is.na(.x)))
...
fish|>
  ...
  summarize(na_weight = sum(is.na(weight)))
...
fish <- fish |> 
  mutate(length = (length - min(length, na.rm = TRUE)) / 
         ...
         weight = (weight - min(weight, na.rm = TRUE)) / 
           (max(weight, na.rm = TRUE) - min(length, na.rm = TRUE)))#error
```
...
The variable called for calculating the weight rescaling is wrong when calculating the minimum. 
